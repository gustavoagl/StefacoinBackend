...
export default class AlunoController {
  ...
  async listar(filtro: FilterQuery<Aluno> = {}): Promise<Aluno[]> {
    const aluno = await AlunoRepository.listar({ tipo: 2 });
    for (let i = 0; i < aluno.length; i++) {
      const cursos = await cursoRepository.obterCursoPorIdAluno(aluno[i].id);
      aluno[i].cursos = cursos;
    }
    return aluno;
  }
  ...
  async incluir(aluno: Aluno) {
    ...
    const emailRepetido: Aluno[] = await new AlunoController().listar({ email: { $eq: email } });
    if (!emailRepetido.length){
